...
|---|---|---|---|
| `refine.yml` | Refinador (4 jobs: `work` + `cleanup` + `quig-work` + `quig-cleanup`, condicionais — ver seção abaixo) | `issue_key` (required), `image` (default `ghcr.io/squadia-ai/core:dev`), `instance`, `timeout_minutes` (dos jobs `work`/`quig-work`, default 75) | `squadia-refine-<issue_key>` |
...
Os seis primeiros também declaram `permissions: id-token: write` no job (além de `contents: read`), necessário pro step opcional de credenciais AWS via OIDC — ver "Credenciais AWS do data plane (OIDC)" abaixo. O `orchestrate-dispatcher.yml` não precisa disso (não toca dataplane). Exceção: `dev.yml`, `qa-scenarios.yml`, `qa-execute.yml` e `review.yml` dividem isso em dois jobs (ver abaixo) — só o job `cleanup` declara `id-token: write`. `refine.yml` é um caso à parte, com quatro jobs — ver parágrafo dedicado abaixo.

`dev.yml`, `qa-scenarios.yml`, `qa-execute.yml` e `review.yml` são reusables com dois jobs em vez de um: `work` roda o agente (no caso do dev: clone, Claude, commit, PR; no caso do qa-scenarios: Claude, subtask de auditoria "QA", transição da issue; no caso do qa-execute: clone, Claude/build/testes, merge do PR; no caso do review: Claude, aprovação/comentário de PR, transição da issue) sem nenhuma credencial AWS, e `cleanup` (`needs: work`, `if: always()`) só libera o lock de conta Claude / atualiza o contador no DynamoDB via OIDC. O motivo é que `aws-actions/configure-aws-credentials` mascara como secret as credenciais temporárias assim que roda, e isso interrompe o live-tail do log do job inteiro no GitHub Actions — separando o step em outro job, o acompanhamento em tempo real do agente (a parte longa e interessante) fica preservado. Os dois jobs trocam informação via artifact (`work-result.json`).

`refine.yml` é o único reusable com **quatro** jobs em vez de dois: além do `work`/`cleanup` do refinador (mesmo motivo acima), ele também despacha o Quig (gerador de cenários de QA) ao final do refinamento, pra fechar a corrida entre o PO aprovar rápido e os cenários existirem (SQD-247). O job `cleanup` do refino, além de liberar o lock/contador de conta do refinador, decide se o Quig deve rodar nesta rodada (tenta o lock da conta dele) e expõe essa decisão como job output (`quig_should_run`). Os jobs `quig-work`/`quig-cleanup` são condicionais a esse output — quando `true`, invocam o entrypoint `qa-scenarios.js` já dividido (o mesmo usado por `qa-scenarios.yml`), reaproveitando a lógica em vez de duplicá-la; quando a conta do Quig está ocupada (ou o tenant não tem o papel "qa" configurado), esses dois jobs nem chegam a rodar — o refino cai no mecanismo residual já existente, gravando o label `aguardando-cenarios-qa` pra faixa "qa_scenarios" do orquestrador complementar depois.
